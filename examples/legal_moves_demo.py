...
from quantik_core import State, generate_legal_moves, generate_legal_moves_list, Move, apply_move
...
def demonstrate_legal_moves():
    ...
    print(f"   Moves per shape: {[(shape, len(moves)) for shape, moves in moves_by_shape.items()]}")
    ...
    current_player, moves_by_shape = generate_legal_moves(state)
    print(f"   Current player: {current_player}")
    print(f"   A moves for player 1: {len(moves_by_shape[0])} (constraints from row/col/zone)")
    print(f"   Other shape moves: B={len(moves_by_shape[1])}, C={len(moves_by_shape[2])}, D={len(moves_by_shape[3])}")
    
    # Show specific A positions that are forbidden/allowed
    a_positions = sorted([move.position for move in moves_by_shape[0]])
    forbidden_positions = sorted(set(range(16)) - set(a_positions) - {0})  # Exclude occupied position 0
    ...
    current_player, moves_by_shape = generate_legal_moves(state)
    print(f"   Current player: {current_player}")
    print(f"   A moves for player 0: {len(moves_by_shape[0])} (already has 2 A pieces - max reached)")
    print(f"   Other moves available: B={len(moves_by_shape[1])}, C={len(moves_by_shape[2])}, D={len(moves_by_shape[3])}")
    print()
    ...
    wrong_player, no_moves = generate_legal_moves(state, player_id=1)  # Player 1 when it's player 0's turn
    print(f"   Requesting moves for player 1 when it's player 0's turn:")
    ...
    for i, move in enumerate(moves_sequence):
        ...
        if is_valid:
            ...
            print(f"   Breaking due to invalid move")
            break
