...
def test_parse_model_checkpoint_manifest_accepts_valid_manifest():
    ...
    assert manifest.contract_version == "1.3.0"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "contract_version",
            "1.0.0",
            "contract_version must match supported contracts release 1.3.0",
        ),
...
def test_observation_parquet_rejects_release_metadata_drift(tmp_path):
    ...
    with pytest.raises(ValueError, match="contracts_release must be 1\\.3\\.0"):
        load_observations_parquet(path)
