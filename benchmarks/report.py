...
def render_markdown(bundle_dict: dict) -> str:
    ...
    checkpoint = bundle_dict.get("checkpoint")
    ...
    if checkpoint:
        ...
        parts.extend(
            [
                f"- checkpoint status: {checkpoint.get('status', 'unknown')}",
                (
                    "- checkpoint counts: "
                    f"observations {counts.get('observations', 0)}, "
                    f"h2h_records {counts.get('h2h_records', 0)}"
                ),
            ]
        )
        if checkpoint.get("h2h_pairs") is not None:
            parts.append(f"- checkpoint h2h pairs: {checkpoint['h2h_pairs']}")
    parts.extend(
        [
            "",
